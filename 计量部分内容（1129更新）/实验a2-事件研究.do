...
forvalues i = 5(-1)1 {
    local j = -`i'
    gen pre_`i' = (rel_year == `j') * intensity
}

forvalues i = 0/1{
    gen post_`i' = (rel_year == `i') * intensity
}

*========================================================
*避免多重共线性，回归模型省略pre_1期
* (1)单位GDP能耗,2017-2022
xtset id year
...
esttab event_能耗 event_能耗增长率 event_非化石能源占比 event_非化石能源占比增长率 event_碳排放 event_碳排放增长率, ///
...
forvalues i = 1/6 {
    ...
    twoway ///
        ...
        (scatter estimate_plot time, mcolor(red) msymbol(O) msize(medium)) ///
        (line estimate_plot time, lcolor(red) lpattern(solid) lwidth(medium)), ///
