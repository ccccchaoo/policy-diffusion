...
*========================================================
* 基准回归结果输出（不含增长率部分）
* (1)单位GDP能耗,2017-2023 - 有控制变量
reghdfe 单位GDP能耗 did 工业结构 城市化水平 人口密度 技术进步 财政自给率, absorb(id year) vce(cluster id)
estimates store did_能耗_控制变量

* (2)单位GDP能耗,2017-2023 - 无控制变量
reghdfe 单位GDP能耗 did, absorb(id year) vce(cluster id)
estimates store did_能耗_无控制变量

* (3)非化石能源占比,2017-2023 - 有控制变量
reghdfe 非化石能源占比 did 工业结构 城市化水平 人口密度 技术进步 财政自给率, absorb(id year) vce(cluster id)
estimates store did_非化石_控制变量

* (4)非化石能源占比,2017-2023 - 无控制变量
reghdfe 非化石能源占比 did, absorb(id year) vce(cluster id)
estimates store did_非化石_无控制变量

* (5)单位GDP碳排放量,2017-2023 - 有控制变量
reghdfe 单位GDP碳排放量 did 工业结构 城市化水平 人口密度 技术进步 财政自给率, absorb(id year) vce(cluster id)
estimates store did_碳排_控制变量

* (6)单位GDP碳排放量,2017-2023 - 无控制变量
reghdfe 单位GDP碳排放量 did, absorb(id year) vce(cluster id)
