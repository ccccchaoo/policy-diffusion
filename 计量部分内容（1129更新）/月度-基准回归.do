...
*ssc install e
import excel "总.xlsx", firstrow clear
...
forval i = 0/8 {
    ...
    forval j = 1/7 {
        if `j' == 1 {
            // 全样本回归
            if `i' == 0 {
                capture reghdfe y did 工业结构 城市化水平 人口密度 技术进步 财政自给率, absorb(id month) vce(cluster id)
            }
            else {
                capture reghdfe y`i' did 工业结构 城市化水平 人口密度 技术进步 财政自给率, absorb(id month) vce(cluster id)
            }
        }
        else if `j' == 2 {
            // 100-75分位数
            if `i' == 0 {
                capture reghdfe y did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high1 == 1, absorb(id month) vce(cluster id)
            }
            else {
                capture reghdfe y`i' did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high1 == 1, absorb(id month) vce(cluster id)
            }
        }
        else if `j' == 3 {
            // 75-50分位数
            if `i' == 0 {
                capture reghdfe y did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high2 == 1, absorb(id month) vce(cluster id)
            }
            else {
                capture reghdfe y`i' did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high2 == 1, absorb(id month) vce(cluster id)
            }
        }
        else if `j' == 4 {
            // 50-25分位数
            if `i' == 0 {
                capture reghdfe y did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high3 == 1, absorb(id month) vce(cluster id)
            }
            else {
                capture reghdfe y`i' did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high3 == 1, absorb(id month) vce(cluster id)
            }
        }
        else if `j' == 5 {
            // 25-0分位数
            if `i' == 0 {
                capture reghdfe y did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high4 == 1, absorb(id month) vce(cluster id)
            }
            else {
                capture reghdfe y`i' did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high4 == 1, absorb(id month) vce(cluster id)
            }
        }
        else if `j' == 6 {
            // 100-50分位数
            if `i' == 0 {
                capture reghdfe y did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high5 == 1, absorb(id month) vce(cluster id)
            }
            else {
                capture reghdfe y`i' did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high5 == 1, absorb(id month) vce(cluster id)
            }
        }
        else if `j' == 7 {
            // 50-0分位数
            if `i' == 0 {
                capture reghdfe y did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high6 == 1, absorb(id month) vce(cluster id)
            }
            else {
                capture reghdfe y`i' did 工业结构 城市化水平 人口密度 技术进步 财政自给率 if intensity_high6 == 1, absorb(id month) vce(cluster id)
            }
        }
...
* 导出到Excel
export excel using "分位数回归结果-月度.xlsx", firstrow(variables) replace
